<!--
agent_page_id: models-and-providers
...
primary_files:
  ...
  - packages/client/src/components/hermes/models/ProvidersPanel.vue
  - packages/server/src/routes/hermes/models.ts
  - packages/server/src/routes/hermes/providers.ts
screenshot_assets:
...
> Agent summary: 模型提供商、credential pool/OAuth、自定义 OpenAI-compatible provider、默认模型和辅助模型。

## What it is

模型提供商、credential pool/OAuth、自定义 OpenAI-compatible provider、默认模型和辅助模型。 本页面把可见 UI、route、源码锚点和截图放在一起，便于人审查，也便于 agent 快速定位。
...
<sub>models management — isolated latest-main product/manual screenshot; no private Han data.</sub>

![models add provider modal](assets/screenshots/models-add-provider-modal.png)

<sub>models add provider modal — isolated latest-main product/manual screenshot; no private Han data.</sub>
...
- 模型显示名/可见性不等于 Hermes provider/model canonical ID；写 config 前保留 canonical ID。
