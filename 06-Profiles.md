...
primary_routes:
  - /hermes/profiles
primary_files:
  - packages/client/src/views/hermes/ProfilesView.vue
...
> Agent summary: Hermes profiles 创建、克隆、导入、导出、切换与 profile-scoped 配置隔离。

## What it is

Hermes profiles 创建、克隆、导入、导出、切换与 profile-scoped 配置隔离。 本页面把可见 UI、route、源码锚点和截图放在一起，便于人审查，也便于 agent 快速定位。
...
![profiles management](assets/screenshots/profiles-management.png)

<sub>profiles management — isolated latest-main product/manual screenshot; no private Han data.</sub>

![profiles create modal](assets/screenshots/profiles-create-modal.png)

<sub>profiles create modal — isolated latest-main product/manual screenshot; no private Han data.</sub>
