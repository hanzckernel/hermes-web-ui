<!--
agent_page_id: coding-agents-global-agent-mcp
...
primary_files:
  ...
  - packages/client/src/views/hermes/McpManagerView.vue
  - packages/server/src/routes/hermes/mcp.ts
...
> Agent summary: Coding agent runner、Global Agent 会话、MCP server 管理和工具注入边界。

## What it is

Coding agent runner、Global Agent 会话、MCP server 管理和工具注入边界。 本页面把可见 UI、route、源码锚点和截图放在一起，便于人审查，也便于 agent 快速定位。
...
<sub>coding agents — isolated latest-main product/manual screenshot; no private Han data.</sub>

![mcp servers](assets/screenshots/mcp-servers.png)

<sub>mcp servers — isolated latest-main product/manual screenshot; no private Han data.</sub>

![mcp add server modal](assets/screenshots/mcp-add-server-modal.png)

<sub>mcp add server modal — isolated latest-main product/manual screenshot; no private Han data.</sub>
