...
> Agent summary: 基于 node-pty/xterm 的网页终端，多 session、resize、WebSocket 输入输出。

## What it is

基于 node-pty/xterm 的网页终端，多 session、resize、WebSocket 输入输出。 本页面把可见 UI、route、源码锚点和截图放在一起，便于人审查，也便于 agent 快速定位。
